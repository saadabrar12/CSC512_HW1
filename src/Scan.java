...
public class Scan {
	...
	public Pair<TokenNames,String> getNextToken() {
		...
		while((ch = getNextChar()) != '\n' && ch != '\u001a') {
			...
			if(foundToken && currentToken == TokenNames.None) {
				...
				//numbers followed by letters means invalid variable name
				if(lastFoundToken == TokenNames.Numbers && isLetter(lastReadChar) ){
					System.err.println("Invalid input Not a valid token: " + characters);
					writer.write("The input program contains errors for scanning");
					writer.close();
					System.exit(1);
				}
